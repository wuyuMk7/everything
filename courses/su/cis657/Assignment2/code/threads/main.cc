...
main(int argc, char **argv)
{
    ...
    for (i = 1; i < argc; i++) {
	    ...
	    userProgs[userProgCount ++] = argv[i + 1];
	    i++;
	}
    ...
    for (int j = 0;j < userProgCount; ++j) {
	Thread *t = new Thread(userProgs[j]);
        t->Fork((VoidFunctionPtr)RunUserProg, (void *)userProgs[j]);
    }
