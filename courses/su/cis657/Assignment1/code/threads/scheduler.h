...
class PriorityScheduler {
  public:
    PriorityScheduler() {};
    PriorityScheduler(int (*)(Process*, Process*));		// Initialize list of ready threads 
    ~PriorityScheduler();		// De-allocate ready list

    void ReadyToRun(Process *process);	
    				// Thread can be dispatched.
    int HighestPriorityInRQ();
    Process* FindNextToRun();	// Dequeue first thread on the ready 
				// list, if any, and return thread.
    void Run(Process *next, bool finishing);
    				// Cause nextThread to start running
    void CheckToBeDestroyed();// Check if thread that had been
    				// running needs to be deleted
    void Print();		// Print contents of ready list
