...
Thread::Thread(char* threadName)
{
    ...
    int len_name = strlen(threadName);
    name = new char[len_name + 1];
    strncpy(name, threadName, len_name + 1);
...
Thread::~Thread()
{
    ...
    //ASSERT(this != this->_owner->currentThread());
...
Thread::Thread(const Thread& t)
{
    ...
    name = new char[len_name + 1];
    strncpy(name, t.name, len_name);

    StackAllocate((VoidFunctionPtr)t.machineState[InitialPCState], t.machineState[InitialArgState]);
}
...
Thread::Fork(VoidFunctionPtr func, void *arg)
{
    ...
    if (this->_owner == NULL)
    	this->_owner = kernel->currentProcess;
    this->_owner->Threads().Append(this);
	...
	" f(a): " << (int) func << 
	" " << arg << 
        " Owner Process: " << this->_owner->GetPid() << endl
    );
    
    StackAllocate(func, arg);

    oldLevel = interrupt->SetLevel(IntOff);
    this->_owner->scheduler()->ReadyToRun(this);	// ReadyToRun assumes that interrupts 
					// are disabled!
...
Thread::Begin ()
{
    ASSERT(this->_owner == kernel->currentProcess);
    ASSERT(this == this->_owner->currentThread());
...
Thread::Finish ()
{
    ...
    this->_owner->Threads().Remove(this);
...
Thread::Yield ()
{
    ...
    Scheduler *scheduler = kernel->currentProcess->scheduler();
...
Thread::Sleep (bool finishing)
{
    ...
    scheduler->Run(nextThread, finishing); 
}
