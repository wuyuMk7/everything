...
class SynchDisk : public CallBackObj {
  public:
    ...
    void ReadBlock(int blockNumber, char* data);
    					// Read/write a disk block, returning
    					// only once the data is actually read 
					// or written.  These call
    					// Disk::ReadRequest/WriteRequest and
					// then wait until the request is done.
    void ReadFragment(int fragmentNumber, char* data);

    void ReadLength(int sectorNumber, int len, char* data);

    void WriteSector(int sectorNumber, char* data);
    void WriteBlock(int blockNumber, char* data);
    void WriteFragment(int fragmentNumber, char* data);

    void WriteLength(int sectorNumber, int len, char* data);
