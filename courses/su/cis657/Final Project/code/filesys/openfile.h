...
class OpenFile {
  public:
    ...
    OpenFile(int inode_number);		// Open a file with its inode number
    ~OpenFile();			// Close the file

    void Seek(int position); 		// Set the position from which to 
    void SeekToEnd(); 			// Set the position to the end
    ...
    int WriteAt(char *from, int numBytes, int position);
    int WriteDir(char *from, int numBytes, int position);
  ...
  private:
    ...
    FileHeader *hdr;			// Header for this file (inode) 
};
